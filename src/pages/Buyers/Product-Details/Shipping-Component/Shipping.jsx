...
const ShippingComponent = ({ quantity, increaseQuantity, decreaseQuantity, addToCart, product }) => {
            ...
            <div className="px-5 py-10 bg-white rounded-lg flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-[10px]">
                    ...
                    <div className="flex gap-3 items-center justify-start">
                        <button type="button" className="bg-gray-300 text-white w-6 h-6 rounded-md" onClick={decrementQty}>-</button>
                        <div className="px-3">{quantity}</div>
                        <button type="button" className="bg-gray-300 text-white w-6 h-6 rounded-md" onClick={incrementQty}>+</button>
                    </div>
                    <p>1 piece at most per customer</p>
                </div>
                <div className="flex flex-col items-stretch gap-4">
                    <button type="button" className="rounded bg-[#EEC643] py-4 px-8 border border-solid border-[#EEC643]" onClick={() => handleCart(product)}>Add To Cart</button>
                    <button type="button" className="text-[#2284B6] py-4 px-8 border-[#2284B6] border-solid border rounded" onClick={handleBuyNow}>Buy It Now</button>
                </div>
            </div>
...
const mapStateToProps = (state) => ({
    quantity: state.shipReducer.quantity || 1, // Default to 1 if undefined
    product: state.product || {}, // Add product state if needed
});
